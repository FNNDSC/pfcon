...
# DESC
#
#   'make.sh' sets up a pfcon development instance running on Docker, Swarm or Kubernetes.
...
# TYPICAL CASES:
#
#   Run full pfcon instantiation operating out-of-network on Docker:
#
...
#
#   Run full pfcon instantiation operating in-network on Docker using mounted filesystem storage:
#
...
#
#       unmake.sh -O swarm; sudo rm -fr CHRIS_REMOTE_FS; rm -fr CHRIS_REMOTE_FS; make.sh -O swarm
#
...
title -d 1 "Setting global exports..."
    ...
    if [[ $CONTAINER_ENV == kubernetes ]]; then
        ...
        VOLUME_NAME=storebase
        echo -e "exporting SCOMPUTE_VOLUME_TYPE=$COMPUTE_VOLUME_TYPE "   | ./boxes.sh
...
title -d 1 "Starting pfcon containerized dev environment on $CONTAINER_ENV"
    if [[ $CONTAINER_ENV == docker ]]; then
        if (( b_pfconInNetwork )) ; then
            ...
            echo "docker compose -f swarm/docker-compose_dev.yml up -d" | ./boxes.sh ${LightCyan}
            docker compose -f swarm/docker-compose_dev.yml up -d
        fi
...
title -d 1 "Waiting until pfcon stack containers are running on $CONTAINER_ENV"
    echo "This might take a few minutes... please be patient."      | ./boxes.sh ${Yellow}
    for i in {1..30}; do
        sleep 5
        if [[ $CONTAINER_ENV == docker ]]; then
            pfcon_dev=$(docker ps -f name=pfcon -q)
        elif [[ $CONTAINER_ENV == swarm ]]; then
