"""
Integration tests for the client-managed job resources in S3 storage mode.
Requires a running Docker daemon and MinIO (S3-compatible) service.

Tests the full lifecycle:
  POST /copyjobs/    -> schedule copy container (fetches from S3)
  ...
  POST /uploadjobs/  -> schedule upload container (pushes to S3)
...
from pfcon.storage.s3manager import S3Manager


class NewResourcesS3Tests(TestCase):
    """Base class for the new resources integration tests in S3 mode."""
    ...
    def setUp(self):
        ...
        with self.app.test_request_context():
            ...
            self.s3_manager = S3Manager(
                self.app.config.get('S3_BUCKET_NAME'),
                self.app.config.get('S3_CONNECTION_PARAMS'))

            self.s3_input_path = 'foo/feed/input'
            self.s3_output_path = 'foo/feed/output'

            # Upload a test file to S3 input path
            self.s3_manager.upload_obj(
                self.s3_input_path + '/test.txt',
                'Test file', content_type='text/plain')

    def tearDown(self):
        # Clean up S3 objects
        for prefix in (self.s3_input_path, self.s3_output_path):
            for obj_path in self.s3_manager.ls(prefix):
                self.s3_manager.delete_obj(obj_path)
...
class TestFullLifecycleS3(NewResourcesS3Tests):
    """Full end-to-end test of the new client-managed job flow with S3."""
    ...
    def test_full_lifecycle(self):
        ...
        try:
            ...
            copy_data = {
                'jid': job_id,
                'input_dirs': [self.s3_input_path],
                'output_dir': self.s3_output_path,
            }
            ...
            response = self.client.get(
                file_url,
                query_string={'job_output_path': self.s3_output_path},
                headers=self.headers)
            ...
            upload_data = {
                'jid': job_id,
                'job_output_path': self.s3_output_path,
            }
            ...
            s3_files = list(self.s3_manager.ls(self.s3_output_path))
            self.assertGreater(len(s3_files), 0,
    ...
    def test_copy_with_chris_links(self):
        """Copy job should recursively follow .chrislink files from S3."""
        job_id = 'new-s3-links-1'

        # Set up ChRIS link structure in S3
        self.s3_manager.upload_obj(
            'PIPELINES/bob/pipeline.yml',
            'Test pipeline', content_type='text/plain')
        self.s3_manager.upload_obj(
            'home/bob/PIPELINES_bob.chrislink',
            'PIPELINES/bob', content_type='text/plain')
        self.s3_manager.upload_obj(
            self.s3_input_path + '/home_bob.chrislink',
            'home/bob', content_type='text/plain')
        ...
        try:
            ...
            if os.path.isdir(key_dir):
                shutil.rmtree(key_dir)
            # Clean up extra S3 objects
            for prefix in ('PIPELINES/bob', 'home/bob'):
                for obj_path in self.s3_manager.ls(prefix):
                    self.s3_manager.delete_obj(obj_path)


class TestUploadIdempotencyS3(NewResourcesS3Tests):
    """Test upload idempotency with real S3."""
...
class TestCopyJobListS3(NewResourcesS3Tests):
    """Test CopyJobList resource with S3."""
    ...
    def test_get_returns_server_info(self):
        ...
        self.assertEqual(response.json['storage_env'], 's3')
        self.assertIn('s3_endpoint_url', response.json)


class TestPluginJobListS3(NewResourcesS3Tests):
    """Test PluginJobList resource with S3."""
...
class TestDeleteJobNoDataS3(NewResourcesS3Tests):
