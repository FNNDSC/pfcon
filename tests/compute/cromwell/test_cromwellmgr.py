...
from pfcon.compute.abstractmgr import Image, JobName
from pfcon.compute.cromwellmgr import CromwellManager, CromwellException, WorkflowId
from .examples import metadata as metadata_example
from .examples import query as query_example
from .helpers import patch_cromwell_api, create_404_response
...
class CromwellTestCase(unittest.TestCase):
    ...
    @patch('time.sleep')
    @patch('pfcon.compute.cromwell.slurm.wdl.SlurmJob.to_wdl')
    ...
    def test_submit(self, mock_submit: Mock, mock_metadata: Mock,
        ...
        # but pfcon wants to get job info, so we need to poll Cromwell a few times.
        ok_res = Mock()
        ...
        self.assertBytesIOEqualDict(
            {CromwellManager.PFCON_CROMWELL_LABEL: 'example-jid-4567'},
    ...
    def test_get_job(self, mock_query: Mock):
        ...
        mock_query.assert_called_once_with(
            query_dict={'label': {self.manager.PFCON_CROMWELL_LABEL: 'sushi'}},
